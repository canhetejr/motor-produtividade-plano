...
1. Criar uma esteira de CI mínima dedicada à suíte de integração MCP. Provedor: **GitHub Actions** — o repositório já vive em `canhetejr/vertice` no GitHub; não introduzir um serviço de CI novo só para isso. Escopo mínimo: um workflow que roda `npm test` incluindo `__tests__/isolamento/mcp-real.integration.test.ts` (a criar no Gate 1) contra o banco de teste do item 0.2.
