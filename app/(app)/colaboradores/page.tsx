import { requireGestor } from '@/lib/auth'
import { createClient } from '@/utils/supabase/server'
import { ColaboradoresManager } from './colaboradores-manager'

export const dynamic = 'force-dynamic'

export default async function ColaboradoresPage() {
  await requireGestor()
  const supabase = await createClient()

  const [{ data: areas }, { data: colaboradores }] = await Promise.all([
    supabase.from('areas').select('*').order('nome'),
    supabase.from('colaboradores').select('*').order('nome'),
  ])

  return (
    <div className="container mx-auto p-4 md:p-8">
      <h1 className="text-3xl font-bold mb-6">Equipe e Colaboradores</h1>
      <p className="text-muted-foreground mb-8">
        Gerencie os perfis, áreas e carga horária de cada colaborador. Novas contas são criadas aqui pelo gestor.
      </p>

      <ColaboradoresManager areas={areas || []} colaboradores={colaboradores || []} />
    </div>
  )
}
