...
const CAMPO = 'h-10 rounded-md text-base md:text-sm'
...
// move durante o envio.
function BarraDeProgresso() {
  const { pending } = useFormStatus()
  if (!pending) return null
  return (
    <div
      className="absolute inset-x-0 top-0 h-0.5 overflow-hidden rounded-t-md"
      role="progressbar"
      aria-label="Entrando"
    >
...
export function LoginForm({ mensagem }: { mensagem?: string }) {
  ...
  return (
    <form action={login} className="flex flex-col gap-4">
      ...
      {mensagem && (
        // role="alert" faz o leitor de tela anunciar o erro na hora; sem isso a
        // mensagem aparece na tela e passa despercebida por quem não a vê.
        <p
          role="alert"
